...
public class EnemyMeleeAttack : EnemyMeleeProcess
{
    ...
    [SerializeField] private bool _isAttacked;              // すでに攻撃したか
    public bool IsAttacked {  get => _isAttacked; }

    [SerializeField] private float _postAttackCounter;      // 攻撃から逃走するまでのカウンタ

    // パラメータ
    [SerializeField] private Vector3 _dashForce;                    // ダッシュ速度
    [SerializeField] private float _attackStartRange;               // 攻撃開始範囲
    [SerializeField] private Vector3 _offsetAttackCollider;         // 攻撃判定のオフセット
    [SerializeField] private GameObject _prefabAttackCollider;      // 攻撃判定のプレハブ
    [SerializeField] private float _postAttackTime;                 // 攻撃から逃走するまでの時間
